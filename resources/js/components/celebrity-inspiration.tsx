...
const CelebrityInspiration: React.FC<CelebrityInspirationProps> = ({
    ...
    return (
        <div className={className}>
            <h2 className="px-4 pt-5 pb-3 text-[22px] leading-tight font-bold tracking-[-0.015em] text-[#181611]">
            ...
            <div className="grid grid-cols-[repeat(auto-fit,minmax(158px,1fr))] gap-3 p-4">
                {items.map((item) => (
                    <div key={item.id} className="flex flex-col gap-3 pb-3">
                        <div
                            className="aspect-square w-full cursor-pointer rounded-lg bg-cover bg-center bg-no-repeat transition-transform duration-300 hover:scale-105"
